...
public class CommonGoalCardsTokenRender {
    /**
     ...
     */
    public static Image generateTokenImageView(CommonGoalCardStatus commonGoalCard) {
        //ImageView tokenImageView = new ImageView();

        Token id = commonGoalCard.getCardTokens().get(commonGoalCard.getCardTokens().size() - 1);

        return new Image(GuiResources.getToken(id));
